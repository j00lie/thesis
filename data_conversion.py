...
def loadCheckedLoadables(self):
    ...
    for plugin in self.loadablesByPlugin:
        for loadable in self.loadablesByPlugin[plugin]:
            if hasattr(loadable, "referencedInstanceUIDs"):
                ...
                if len(instanceFileList) and not self.isFileListInCheckedLoadables(
                    instanceFileList
                ):
    ...
    if len(referencedFileLists):
        (self.referencedLoadables, loadEnabled) = self.getLoadablesFromFileLists(
            referencedFileLists
        )

    automaticallyLoadReferences = int(
        slicer.util.settingsValue(
            "DICOM/automaticallyLoadReferences", qt.QMessageBox.InvalidRole
        )
    )
...
def import_T1_and_T2_data(input_folder, case_number):
    ...
    DICOMUtils.importDicom(
        os.path.join(input_folder, patient_dir1)
    )  # import T1 folder files
    DICOMUtils.importDicom(
        os.path.join(input_folder, patient_dir2)
    )  # import T2 folder files
    ...
    patient = slicer.dicomDatabase.patients()[
        0
    ]  # select the patient from the database (which has only one patient)
    ...
    for key in loadables:
        ...
        if "RTSTRUCT" in name or (
            ("t1_" in name or "t2_" in name) and not " MR " in name
        ):
    ...
    )  # gets the nodes that correspond to RT structures

    assert (
        len(ns) == 2
    ), f"Not exactly 2, but {len(ns)} node of class vtkMRMLSegmentationNode."
...
def register_and_resample(
    input_node, reference_node, transform_node=None, interpolationMode="Linear"
):
    ...
    if transform_node:
        ...
        copy_input_node = slicer.modules.volumes.logic().CloneVolume(
            slicer.mrmlScene, input_node, "translated"
        )
        ...
        copy_input_node = slicer.modules.volumes.logic().CloneVolume(
            slicer.mrmlScene, input_node, "copy"
        )

    # resample volume
    registered_and_resampled_node = slicer.mrmlScene.AddNewNodeByClass(
        copy_input_node.GetClassName()
    )
    parameters = {
    ...
    slicer.cli.run(
        slicer.modules.brainsresample, None, parameters, wait_for_completion=True
    )
    slicer.mrmlScene.RemoveNode(copy_input_node)  # remove temporary copy of input node
    registered_and_resampled_node.SetName(
        input_node.GetName() + "_registered_and_resampled"
    )
...
def create_segmentation_node_with_reference_geometry(name, ref_geometry_image_node):
    new_segmentation_node = slicer.mrmlScene.AddNewNodeByClass(
        "vtkMRMLSegmentationNode"
    )
    new_segmentation_node.CreateDefaultDisplayNodes()
    new_segmentation_node.SetReferenceImageGeometryParameterFromVolumeNode(
        ref_geometry_image_node
    )
...
def create_segments_from_structure_contour_list(
    segmentationNode, structure_contour_list
):
    ...
    for i, struc in enumerate(structure_contour_list):
        ...
        createSegNodeFromContourPoints(
            segmentationNode, contours_RAS, struc["structure_name"]
        )


def save_labelmaps_from_planar_contour(
    planar_contour_segmentation_node,
    ref,
    export_only_tumour_seg,
    case_number,
    output_folder,
):
    ...
    for segmentIndex in range(0, nb_structures):
        ...
        slicer.modules.segmentations.logic().ExportSegmentsToLabelmapNode(
            pc_node, segmentID_a, lm_node, ref
        )
...
def main(argv):
    ...
    if args.register not in ["no_registration", "T1", "T2"]:
        logging.error(
            'Invalid value for keyword "--register": choose "T1" or "T2" or "no_registration"'
        )
    ...
    for i in range(len(patient_dirs)):
        ...
        if register_T1_image_and_contour_points_to_T2_image:
            transform_path = os.path.join(
                input_folder, f"vs_gk_{case_number}" + "_t1", "inv_T1_LPS_to_T2_LPS.tfm"
            )
            transformNode = slicer.util.loadNodeFromFile(
                transform_path, filetype="TransformFile"
            )
            ...
            transform_path = os.path.join(
                input_folder, f"vs_gk_{case_number}" + "_t2", "inv_T2_LPS_to_T1_LPS.tfm"
            )
            transformNode = slicer.util.loadNodeFromFile(
                transform_path, filetype="TransformFile"
            )
        ...
        create_segments_from_structure_contour_list(
            segmentationNode_T1, structure_contour_list_T1
        )
        create_segments_from_structure_contour_list(
            segmentationNode_T2, structure_contour_list_T2
        )
        ...
        if (
            not register_T1_image_and_contour_points_to_T2_image
            and not register_T2_image_and_contour_points_to_T1_image
        ):
            lm_node_in_T2_list = save_labelmaps_from_planar_contour(
                segmentationNode_T2,
                ref2,
        ...
        if register_T1_image_and_contour_points_to_T2_image:
            slicer.util.saveNode(
                ref1,
                os.path.join(
                    output_folder, f"vs_gk_{case_number}", f"vs_gk_t1_refT2.nii.gz"
                ),
            )  # pass vol node and destination filename
            slicer.util.saveNode(
                ref2,
                os.path.join(
                    output_folder, f"vs_gk_{case_number}", f"vs_gk_t2_refT2.nii.gz"
                ),
            )  # pass vol node and destination filename
        if register_T2_image_and_contour_points_to_T1_image:
            slicer.util.saveNode(
                ref1,
                os.path.join(
                    output_folder, f"vs_gk_{case_number}", f"vs_gk_t1_refT1.nii.gz"
                ),
            )  # pass vol node and destination filename
            slicer.util.saveNode(
                ref2,
                os.path.join(
                    output_folder, f"vs_gk_{case_number}", f"vs_gk_t2_refT1.nii.gz"
                ),
            )  # pass vol node and destination filename
